...
type PutOptions = { access: "public"; addRandomSuffix: false; allowOverwrite: boolean; contentType: string; cacheControlMaxAge: number };
...
/**
 * Upload the index in `dir` (manifest.json, meta.json, vectors.bin) to Vercel Blob under
 ...
 */
export async function uploadCardIndex(dir: string, put: BlobPut): Promise<{ prefix: string; baseUrl: string }> {
  ...
  const options = (contentType: string): PutOptions => ({ access: "public", addRandomSuffix: false, allowOverwrite: true, contentType, cacheControlMaxAge: ONE_YEAR });
  await Promise.all([
