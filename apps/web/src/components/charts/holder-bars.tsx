 ...
 * ("sharded 14:02", "settled 14:22", "now"). Plain divs.
 */
export function HolderBars({ points, shardedAt, settledAt, height = 128 }: {
  ...
  return (
    <ChartFrame
      ...
      table={{ columns: ["Time", "Holders"], rows: points.map((p) => [hhmm(p.t), p.holders]) }}
    >
      <div className="flex flex-col gap-3">
        <div className="flex items-end gap-1.5" style={{ height }} role="img" aria-label={`Holders over time, now ${points.at(-1)?.holders ?? 0}`}>
          {points.map((p, i) => (
            <span
              key={p.t}
              title={`${hhmm(p.t)}: ${p.holders} holder${p.holders === 1 ? "" : "s"}`}
        ...
        <div className="flex justify-between font-mono text-[11px] text-muted-foreground" suppressHydrationWarning>
          <span suppressHydrationWarning>{start != null ? `sharded ${hhmm(start)}` : ""}</span>
          {settledAt != null && <span suppressHydrationWarning>settled {hhmm(settledAt)}</span>}
          <span>now</span>
        </div>
      </div>
    </ChartFrame>
