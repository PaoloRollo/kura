...
export function RedemptionMeter({ value, threshold = 0.8, label = "Redemption", className, ...props }: RedemptionMeterProps) {
    ...
    <div data-slot="redemption-meter" data-eligible={eligible || undefined} className={cn("flex w-full flex-col gap-1.5", className)} {...props}>
      <div className="flex items-start justify-between text-[12px]">
        ...
        <span className={eligible ? "text-kin" : "text-text-2"}>
          {pct}% · {eligible ? "eligible" : `needs ${need}%`}
        </span>
      </div>
      <div
        role="meter"
      ...
      >
        <div className={cn("absolute inset-y-0 left-0 rounded-xs", eligible ? "bg-kin" : "bg-s1")} style={{ width: `${v * 100}%` }} />
