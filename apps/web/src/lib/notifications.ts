  ...
  for (const auction of new Set(myOpen.map((b) => lc(b.auction)))) {
    ...
    if (latest && best.maxPriceQ96 < latest.clearingPriceQ96) {
      ...
      out.push({
        ...
        body: `Clearing ${money(q96ToUsdcPerShard(latest.clearingPriceQ96))} passed your ${money(q96ToUsdcPerShard(best.maxPriceQ96))} max`,
    ...
    if (left > 0n && left <= ENDS_SOON_BLOCKS) {
      ...
      out.push({
        ...
        body: `You're in at ${money(q96ToUsdcPerShard(clearingQ96))} per shard`,
