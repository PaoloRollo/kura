 ...
 * `multibaas`: the 24h tiles and the recent-events panel from MultiBaas; `multibaas-new`: just linked, nothing held yet.
 */
export default async function AnalyticsDesignPage({ searchParams }: { searchParams: Promise<{ state?: string; range?: string }> }) {
  ...
  return <AnalyticsPreview key={s} state={s} now={requestTime()} initialRange={(s === "quiet-24h" || s.startsWith("multibaas")) && !range ? "24h" : parseRange(range)} />;
}
